`push` 指令做的事是把你的本地提交上传到中央仓库去，用本地的内容来覆盖掉远端的内容。
...
![本地仓库超前于远程仓库](assets/160071880ab72f00tplv-t2oaga2asx.jpg)
...
![把 master push 到远程仓库](assets/1600725e9973f71dtplv-t2oaga2asx.gif)

而如果这时候我再切到 `feature1` 去后再执行一次 `push`，就会把 `feature1` 以及它的 `commit` `4` 上传到远程仓库：

```shell
git checkout feature1 git push origin feature1
```
...
![push feature1 到远程仓库](assets/160073ccda56ef07tplv-t2oaga2asx.gif)
